...
const ScrapeForm: React.FC<ScrapeFormProps> = ({ onResults }) => {
  ...
  const extractMovieName = (url: string): string => {
    try {
      ...
      // Try to extract from movie-details path (Reel Cinemas format)
      const detailsIndex = pathParts.findIndex(part => part === 'movie-details' || part.includes('movie-detail'));
      if (detailsIndex !== -1 && detailsIndex < pathParts.length - 2) {
        // Get the movie name part (usually the last part)
